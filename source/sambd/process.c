...
static int switch_message(int type,char *inbuf,char *outbuf,int size,int bufsize)
{
  ...
  else
  {
	...
	DEBUG(3,("joph correct username is in current_user_info \n"));

    /* does this protocol need to be run as root? */
    if (!(flags & AS_USER)) {
	  DEBUG(3,("joph flags=%d AS_USER=%d\n",flags,AS_USER));
      change_to_root_user();
    }
    ...
    if ((flags & AS_USER) && !change_to_user(conn,session_tag)) {
		DEBUG(3,("joph flags=%d AS_USER=%d\n",flags,AS_USER));
      if (flags & AS_GUEST) {
	  	DEBUG(3,("joph flags & AS_GUEST"));
        flags &= ~AS_USER;
      } else {
      	DEBUG(3,("joph else flags & AS_GUEST"));
    ...
    if ((flags & NEED_WRITE) && !CAN_WRITE(conn)) {
		DEBUG(3,("joph flags=%d NEED_WRITE=%d\n",flags,NEED_WRITE));
      return(ERROR_DOS(ERRSRV,ERRaccess));
    }

    /* ipc services are limited */
    if (IS_IPC(conn) && (flags & AS_USER) && !(flags & CAN_IPC)) {
		DEBUG(3,("joph flags=%d CAN_IPC=%d\n",flags,CAN_IPC));
      return(ERROR_DOS(ERRSRV,ERRaccess));	    
    }
	...
	DEBUG(3,("joph flags=%d AS_GUEST=%d\n",flags,AS_GUEST));
	  return(ERROR_DOS(ERRSRV,ERRaccess));
    }
  ...
  DEBUG(3,("joph smb_fn_name=%s outbuf=%s outsize=%d\n",smb_fn_name(type),outbuf,outsize));
