...
#define __STDIO_SET_USER_LOCKING(_stream) ((stream)->__user_locking = 1)
#define attribute_hidden 
/**********************************************************************/
/* Prototypes for internal functions. */

int __parsepwent(void *pw, char *line) attribute_hidden;
int __parsegrent(void *gr, char *line) attribute_hidden;
int __parsespent(void *sp, char *line) attribute_hidden;

int __pgsreader(int (*__parserfunc)(void *d, char *line), void *data,
			   char *__restrict line_buff, size_t buflen, FILE *f);

gid_t* __getgrouplist_internal(const char *user, gid_t gid, int *ngroups) attribute_hidden;
...
static pthread_mutex_t pwfmylock = PTHREAD_MUTEX_INITIALIZER;
static FILE *pwf /*= NULL*/;
void setpwent(void)
{
	pthread_mutex_lock(&pwfmylock);
	if (pwf) {
		rewind(pwf);
	}
	pthread_mutex_unlock(&pwfmylock);
}
...
/**********************************************************************/
static pthread_mutex_t grfmylock = PTHREAD_MUTEX_INITIALIZER;

static FILE *grf /*= NULL*/;

void setgrent(void)
{
	pthread_mutex_lock(&grfmylock);
	if (grf) {
		rewind(grf);
	}
	pthread_mutex_unlock(&grfmylock);
}
...
/**********************************************************************/
#ifdef L_getspent_r
static pthread_mutex_t spfmylock = PTHREAD_MUTEX_INITIALIZER;

static FILE *spf /*= NULL*/;

void setspent(void)
{
	pthread_mutex_lock(&spfmylock);
	if (spf) {
		rewind(spf);
	}
	pthread_mutex_unlock(&spfmylock);
}
	...
	do {
		...
		if (i < 2) {
			...
			*((gid_t *) p) = strtoul(line, &endptr, 10);
			...
			/* Now align (p+1), rounding up. */ /* Assumes sizeof(char **) is a power of 2. */
